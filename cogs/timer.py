...
class timer(commands.Cog):
    def __init__(self, client):
		...
		"""
            ...
            await ctx.send(
                f"Your countdown has been started for {str(seconds)} seconds")
            ...
            message = await ctx.send(
                f"Time Remaining : {str(seconds)} second(s)")

            while True:
                seconds -= 2.5  #increased the update duration to prevent rate-limiting
                if seconds == 0:
                ...
                await message.edit(
                    content=(f"Time Remaining : {str(seconds)} second(s)"))

                # Sleep for 2.5 second
		...
		"""
            second = 0
            await ctx.send(
                "Your stopwatch has been started...Send 'stop', to stop the watch."
            )
            ...
            while True:
                ...
                await message.edit(
                    content=(f"Time elapsed : {second} second(s)"))
                try:
                    await client.wait_for("message",
                                          check=lambda x: x.author == ctx.
                                          author and "stop" in x.content,
                                          timeout=1)
                except asyncio.TimeoutError:
                    continue
                else:
                    await ctx.send(content=(
                        f"Total time elapsed : {str(second)} second(s)"))
                    break
		...
		"""
            ...
            await ctx.send(ctx.author.mention +
                           f" Task: {task}, saved successfully!")
		...
		"""
            ...
            for _i in range(3):
                try:
                    ...
                    if reply.content.lower() in [
                        ...
                        break
                            ...
                            "no", "nope", "not yet", "not done", "incomplete"
                    ]:
                        await ctx.send(
                            ctx.author.mention +
                        ...
                        if _i < 2:
                            await ctx.send(
                                ctx.author.mention +
                                " I couldn't understand you! Type 'yes' or 'yep' if you're done, or 'no' or 'nope' if you aren't!"
                            )
                        else:
                            await ctx.send(
                                ctx.author.mention +
                                "I'm sorry, I couldn't understand you! I'll come back in an hour to remind you again!"
                            )
                            break
                ...
                # Continue in case of timeout
                    ...
                    await ctx.send(ctx.author.mention +
                                   " I'll remind you again in an hour")
                    break
		...
		"""
            ...
            message = await ctx.send(
                "Your countdown has been started for 25 minutes")
            while True:
                time -= 2.5
                if time == 0:
                    await ctx.send(ctx.message.author.mention +
            ...
            time = 300
            await message.edit(
                content=("Your break countdown has been started for 5 minutes")
            )
            while True:
                time -= 2.5
                if time == 0:
		...
		"""
            ...
            AVAILABLE_TAGS = [
                'business', 'education', 'faith', 'famous-quotes',
                'friendship', 'future', 'happiness', 'history',
                'inspirational', 'life', 'literature', 'love', 'nature',
                'politics', 'proverb', 'religion', 'science', 'success',
                'technology', 'wisdom'
            ]
